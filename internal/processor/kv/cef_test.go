...
var cefMessage5 = `CEF:0|Cool Vendor|Cool Product|1.0|FLAKY_EVENT|Something flaky happened.|3|requestClientApplication=Go-http-client/1.1 blank2= blank3= blank4= sourceAddress=127.0.0.1 blank=`
...
func TestParseCef5(t *testing.T) {
	expectedVersion := "0"
	expectedDeviceVendor := "Cool Vendor"
	expectedDeviceProduct := "Cool Product"
	expectedDeviceEventClassId := "FLAKY_EVENT"

	if _, err := parseCef(cefMessage5); err != nil {
		t.Errorf("failed to parse CEF message: %v", err)
	}

	cefObj, err := cefStringToObject(cefMessage5)
	...
	if cefObj.Version != expectedVersion {
		t.Errorf("cefStringToObject(message5).Version got %s; expected %s", cefObj.Version, expectedVersion)
	}

	if cefObj.DeviceVendor != expectedDeviceVendor {
		t.Errorf("cefStringToObject(message5).DeviceVendor got %s; expected %s", cefObj.DeviceVendor, expectedDeviceVendor)
	}

	if cefObj.DeviceProduct != expectedDeviceProduct {
		t.Errorf("cefStringToObject(message5).DeviceProduct got %s; expected %s", cefObj.DeviceProduct, expectedDeviceProduct)
	}

	if cefObj.DeviceEventClassId != expectedDeviceEventClassId {
		t.Errorf("cefStringToObject(message5).DeviceEventClassId got %s; expected %s", cefObj.DeviceEventClassId, expectedDeviceEventClassId)
	}
}
